...
public:
	NewGameRoom(user_iter* InHostUserIter, NewGameRoom* InLeft, NewGameRoom* InRight);
	//for pDoor
	NewGameRoom(const int& InBuffer);
	~NewGameRoom();

	//GameRoom(const GameRoom&) = delete; 
	NewGameRoom & operator=(const NewGameRoom&) = delete;
